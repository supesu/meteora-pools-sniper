...
func InitializeGRPCServer() *grpc.Server {
	...
	memoryEventPublisher := events.NewMemoryEventPublisher(logger)
	processTransactionUseCase := usecase.NewProcessTransactionUseCase(transactionRepository, memoryEventPublisher, logger)
	getTransactionHistoryUseCase := usecase.NewGetTransactionHistoryUseCase(transactionRepository, logger)
	subscriptionRepository := repository.NewSubscriptionRepository()
	manageSubscriptionsUseCase := usecase.NewManageSubscriptionsUseCase(subscriptionRepository, memoryEventPublisher, logger)
	discordConfig := ProvideDiscordConfig(config)
	bot := discord.NewBot(discordConfig, logger)
	notifyTokenCreationUseCase := usecase.NewNotifyTokenCreationUseCase(bot, logger)
	meteoraRepository := repository.NewMeteoraRepository(logger)
	processMeteoraEventUseCase := usecase.NewProcessMeteoraEventUseCase(meteoraRepository, memoryEventPublisher, logger)
	notifyMeteoraPoolCreationUseCase := usecase.NewNotifyMeteoraPoolCreationUseCase(bot, logger)
	dependencies := ProvideGRPCDependencies(config, logger, processTransactionUseCase, getTransactionHistoryUseCase, manageSubscriptionsUseCase, notifyTokenCreationUseCase, processMeteoraEventUseCase, notifyMeteoraPoolCreationUseCase, subscriptionRepository)
	server := grpc.NewServer(dependencies)
	return server
}
...
var ApplicationSet = wire.NewSet(repository.NewTransactionRepository, repository.NewSubscriptionRepository, repository.NewMeteoraRepository, usecase.UseCaseSet, discord.NewBot, grpc.NewServer, events.NewMemoryEventPublisher, ProvideLogger,
	ProvideConfig,
	ProvideDiscordConfig,
	ProvideGRPCDependencies, wire.Bind(new(domain.TransactionRepository), new(*repository.TransactionRepository)), wire.Bind(new(domain.SubscriptionRepository), new(*repository.SubscriptionRepository)), wire.Bind(new(domain.MeteoraRepository), new(*repository.MeteoraRepository)), wire.Bind(new(domain.DiscordNotificationRepository), new(*discord.Bot)), wire.Bind(new(domain.EventPublisher), new(*events.MemoryEventPublisher)),
)
